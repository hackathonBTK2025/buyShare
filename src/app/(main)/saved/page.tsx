...
export default function SavedPage() {
  ...
  const initialSavedPosts = products.slice(0, 4);
  const [savedPosts, setSavedPosts] = useState<Product[]>(initialSavedPosts);

  const handleRemove = (productId: string) => {
    setSavedPosts((prev) => prev.filter((p) => p.id !== productId));
  };
